...
void parentProcess(TCHAR* path)
{
	...
	while (1)
	{
		...
		case '+':
		{
			...
			swprintf(processAddress, 40, L"%d_Process", canPrintEventVector.size()+1);
			canPrintEventVector.push_back(CreateEvent(NULL, TRUE, FALSE, processAddress));
			//std::cout << canPrintEventVector.back() << std::endl;

			//swprintf(processAddress, 40, L"%d Process", canPrintEventVector.size());
...
void childProcess(int processID)
{
	...
	//wprintf(buffer);
	swprintf(buffer, 40, L"%d_Process", processID);
	//wprintf(buffer);
